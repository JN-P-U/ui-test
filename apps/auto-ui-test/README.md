...
## 🎥 이벤트 자동 기록 모드 (EventRecorder)

수동 캡처 없이 사용자의 동작을 자동으로 수집하고, 클릭 시점마다 스크린샷을 캡처하여 증빙 엑셀을 생성하는 모드입니다.

### 동작 원리

화면 우측 하단에 고정된 플로팅 컨트롤러가 DOM 이벤트를 전역으로 감지합니다.

| 이벤트 | 처리 방식 |
|--------|-----------|
| **클릭** (`click`) | 이벤트 저장 + 클릭 직후 전체 화면 자동 캡처 (`html-to-image`) |
| **입력 변경** (`change`) | 이벤트와 입력값 저장 (스크린샷 없음) |
| **Enter 입력** (`keydown Enter`) | 텍스트 입력 필드에서 Enter 시 입력값 즉시 기록 |

> 컨트롤러 자체의 클릭은 `data-event-recorder` 속성으로 필터링되어 기록에서 제외됩니다.  
> 클릭 캡처 중에는 컨트롤러 UI가 화면에서 사라진 뒤 촬영되므로 이미지에 포함되지 않습니다.

### 사용 흐름

1. **`▶ 기록 시작`** 버튼 클릭 → DOM 이벤트 리스닝 시작
2. 앱을 정상적으로 조작 → 클릭·입력이 순서대로 자동 수집
3. **`⏹ 기록 중 (N)`** 버튼 클릭 → 기록 중지
4. **`이벤트 로그 (N) →`** 버튼 클릭 → 모달 오픈
5. 서비스코드(필수), 화면ID, 화면명, 작성자, 업무분류, Level1~4 입력
6. 불필요한 이벤트 개별 삭제 → 남은 항목 기준으로 순번 자동 재정렬 (01, 02, 03…)
7. **`엑셀 다운로드`** 클릭 → XLSX 생성 및 다운로드, 완료 시 목록 초기화
...
| **개정이력** | V1.0 최초 작성, 개정자 |
| **이벤트 로그** | 업무분류/Level/작성자 + 전체 이벤트 (순번·시간·유형·대상요소·설명·URL·캡처여부) |
| **공통체크리스트** | 13개 공통 UI 점검 항목 |
| **이벤트 결과 증빙** | 클릭 이벤트의 스크린샷을 순번별로 삽입 |

### 이미지 미리보기

모달의 이벤트 목록에서 썸네일을 클릭하면 캡처 이미지를 전체 화면으로 확인할 수 있습니다. 오버레이 배경 클릭 또는 ✕ 버튼으로 닫습니다.
